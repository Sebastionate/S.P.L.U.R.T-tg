...
/datum/status_effect/chem/enthrall
	...
	var/lewd = FALSE
	/// SPLURT: whether distance-based withdrawal moods apply
	var/distance_mood_enabled = TRUE
	/// SPLURT: skip mindshield resistance ticks when TRUE (Mk.2)
...
/datum/status_effect/chem/enthrall/on_apply()
	...
	if(HAS_TRAIT(enthrall_victim, TRAIT_PET_SKILLCHIP))
		// Subtypes (Mk.2) may already resolve imprint data before calling parent.
		if(isnull(enthrall_mob))
			var/obj/item/organ/brain/neopet_brain = enthrall_victim.get_organ_slot(ORGAN_SLOT_BRAIN)
			for(var/obj/item/skillchip/mk2pet/mk2_chip in neopet_brain?.skillchips)
				if(mk2_chip.active)
			...
			if(isnull(enthrall_mob))
				for(var/obj/item/skillchip/mkiiultra/neopet_chip in neopet_brain?.skillchips)
					if(!neopet_chip.active || isnull(neopet_chip.enchanter))
						continue
					enthrall_mob = neopet_chip.enchanter
					enthrall_ckey = neopet_chip.enchanter.ckey
					enthrall_gender = neopet_chip.title || "Master"
					lewd = TRUE
					break
		if(isnull(enthrall_mob))
			...
			return FALSE
	else if(isnull(enthrall_mob))
		var/datum/reagent/mkultra/enthrall_chem = locate(/datum/reagent/mkultra) in enthrall_victim.reagents?.reagent_list
		if(isnull(enthrall_chem))
			stack_trace("chem/enthrall applied without reagent imprint or pre-set enthrall_mob.")
			owner.remove_status_effect(src)
			return FALSE
		// Current Bubber reagent stores the enchanter mob; keep legacy ckey keys for admin-bussed cases.
		if(istype(enthrall_chem.data["enchanter"], /mob/living))
			enthrall_mob = enthrall_chem.data["enchanter"]
			enthrall_ckey = enthrall_mob.ckey
			enthrall_gender = enthrall_chem.data["enthrall_gender"] || "Master"
		else if(enthrall_chem.data["enthrall_ckey"])
			...
			message_admins("WARNING: FermiChem: No enthrall_mob found in thrall chem data.")
			stack_trace("No enthrall_mob found in thrall chem data.")
			owner.remove_status_effect(src)
			return FALSE
...
/datum/status_effect/chem/enthrall/tick(seconds_between_ticks)
	...
	//mindshield check
	if(HAS_TRAIT(enthrall_victim, TRAIT_MINDSHIELD) && !ignore_mindshield)//If you manage to enrapture a head, wow, GJ. (resisting gives a bigger bonus with a mindshield) From what I can tell, this isn't possible.
		resistance_tally += 2
		if(prob(10))
	...
	switch(phase)
		...
		if(FULLY_ENTHRALLED)
			if((resistance_tally >= 96 && withdrawl_progress >= 72) || (HAS_TRAIT(enthrall_victim, TRAIT_MINDSHIELD) && !ignore_mindshield && (resistance_tally >= 48)))
				enthrall_tally = 0
				phase = PARTIALLY_ENTHRALLED
				resistance_tally = 0
				resist_modifier = 0
		...
		if(OVERDOSE_ENTHRALLED) //mindbroken
			if(mental_capacity >= 499 && (owner.get_organ_loss(ORGAN_SLOT_BRAIN) <= 0 || (HAS_TRAIT(enthrall_victim, TRAIT_MINDSHIELD) && !ignore_mindshield)) && !owner.reagents.has_reagent(/datum/reagent/mkultra))
				phase = PARTIALLY_ENTHRALLED
				mental_capacity = 500
	...
	if(withdrawl_active == TRUE)//Your minions are really REALLY needy.
		switch(withdrawl_progress)//denial
			...
			if(73 to 95) // 05:00-7:00 - anger
				if(prob(10))
					addtimer(CALLBACK(enthrall_victim, TYPE_PROC_REF(/mob/living, set_combat_mode), TRUE), 2)
					addtimer(CALLBACK(enthrall_victim, TYPE_PROC_REF(/mob, click_random_mob)), 2)
					if(lewd)
	...
	if(status)
		if(status == "Antiresist")
			...
			if(status_strength < 0)
				...
				status_strength -= 1
				owner.heal_overall_damage(brute = 4, burn = 4)
...
/datum/status_effect/chem/enthrall/proc/owner_resist()
	...
	else if(owner.mind?.antag_datums?.len)
		delta_resist *= 1.2

	//role resistance
	//Chaplains are already brainwashed by their god
	if(owner.mind?.assigned_role?.title == "Chaplain")
		delta_resist *= 1.2
	//Chemists should be familiar with drug effects
	if(owner.mind?.assigned_role?.title == "Chemist")
		delta_resist *= 1.2
...
/// Intermediate type used by modular_commands / Mk.2 pet chip.
