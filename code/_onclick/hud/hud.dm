...
/datum/hud/proc/show_hud(version = 0, mob/viewmob)
	...
	var/list/group_extra = screen_groups[HUD_GROUP_EXTRA_INVENTORY] // SPLURT EDIT - Extra inventory
	...
	switch (display_hud_version)
		if (HUD_STYLE_STANDARD)
			...
			// SPLURT EDIT - Extra inventory (requires both inventory and extra toggles)
			if (length(group_extra) && screenmob.hud_used?.inventory_shown && screenmob.hud_used?.extra_shown)
				screenmob.client.screen += group_extra
			//
